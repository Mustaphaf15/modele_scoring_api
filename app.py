...
OPTIMAL_THRESHOLD = config.get("OPTIMAL_THRESHOLD", 0.5)  # Valeur par défaut 0.5 si non définie

# Charger le modèle avec joblib
if not os.path.exists(MODEL_FILE):
    raise FileNotFoundError(f"Le fichier modèle spécifié ({MODEL_FILE}) est introuvable.")
...
def list_id():
    if not os.path.exists(CSV_FILE):
        raise HTTPException(status_code=404, detail="Le fichier CSV spécifié est introuvable.")
    
    df = pd.read_csv(CSV_FILE)
    if 'SK_ID_CURR' not in df.columns:
        raise HTTPException(status_code=400, detail="La colonne 'SK_ID_CURR' est absente du fichier CSV.")
    
    return {"ids": df['SK_ID_CURR'].tolist()}
...
def predict(client_id: int):
    ...
    if not all(feature in client_features.columns for feature in model.feature_names_in_):
        missing_features = [feature for feature in model.feature_names_in_ if feature not in client_features.columns]
        raise HTTPException(status_code=400, detail=f"Les features suivantes sont manquantes: {', '.join(missing_features)}")

    # Effectuer la prédiction en utilisant les probabilités pour la classe positive
    pred_proba = model.predict_proba(client_features)[:, 1]  # Probabilité de la classe positive
    ...
    return {"client_id": client_id, "prediction": int(prediction[0]), "probability": pred_proba[0]}
...
async def get_client_info(client_id: int):
    try:
        ...
        df = create_new_features(df)
        client_data = data[data["id"] == client_id]
        if client_data.empty:
            raise ValueError(f"Aucune donnée trouvée pour le client ID {client_id}.")
